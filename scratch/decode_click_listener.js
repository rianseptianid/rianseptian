...
let sub = content.substring(308000, 308800);
console.log('Raw:', sub);
// Decode strings:
sub = sub.replace(/_0x[a-f0-9]+\((0x[a-f0-9]+)(?:,\s*['"][^'"]*['"])?\)/g, (match, hex, key) => {
  try {
    if (match.startsWith('_0x132d9f')) {
      const kMatch = match.match(/,\s*['"]([^'"]+)['"]/);
      if (kMatch) return JSON.stringify(ctx._0x3ea1(parseInt(hex, 16), kMatch[1]));
    }
