...
// Find occurrences of decoded index for '#inter'
// Let's find which index gave '#inter' in _0x5374
const { ctx, arr } = (() => {
...
for (let i = 0; i < arr.length; i++) {
  try {
    const s = ctx._0x4fe8(i + 0x153);
    if (s.startsWith('#inter') || s.startsWith('inter') || s === '#btnOpenAddInteractionModal') {
      const hex = '0x' + (i + 0x153).toString(16);
      console.log(hex, s);
    }
  } catch(e){}
}
