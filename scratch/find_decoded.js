...
const arr = ctx._0x5374();
const decoded = [];
for (let i = 0; i < arr.length; i++) {
  try {
    const s = ctx._0x4fe8(i + 0x153);
    if (s && (s.includes('trigger') || s.includes('act') || s.includes('inter') || s.includes('List') || s.includes('Modal'))) {
      decoded.push(s);
    }
  } catch(e){}
}

console.log('Decoded matching strings:', decoded);
