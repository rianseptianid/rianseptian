...
// In scratch/decoded_interactions.js, look at the very beginning:
// cc(0x2294)+"tionMo"+"dal")?.["addEve"+'ntList'+'ener']("...
// Let's decode offset 309000 to 310500!
let sub = content.substring(309000, 310500);
sub = sub.replace(/_0x[a-f0-9]+\((0x[a-f0-9]+)(?:,\s*['"][^'"]*['"])?\)/g, (match, hex) => {
  try {
    return JSON.stringify(ctx._0x4fe8(parseInt(hex, 16)));
  } catch(e) {
    return match;
  }
});

console.log('Decoded open button block:');
console.log(sub.substring(0, 1500));
